...
10. ✅ **Storyboard review UI** (COMPLETED 2026-06-29) — New `StoryboardTab` component (`apps/web/src/components/StoryboardTab.tsx`) renders the structured story plan as a hierarchical tree (chapters → scenes → beats → panels) with expand/collapse, level-colored badges, and debounced inline editing of section titles and summaries. New PATCH `/api/sections/[id]` endpoint persists edits. Wired as a 5th tab in `ProjectDetail` (Canvas, Chapters, Storyboard, Knowledge, Settings) plus the floating canvas tab bar. The data was already fetched by `getProjectDetail` (story_sections + panels) but had no dedicated review UI.
...
16. ✅ **Add per-panel regeneration action** (COMPLETED 2026-06-29) — The `/api/panels/[id]/regenerate` endpoint already provides per-panel regeneration: it fetches the panel, populates `referenceImageKeys` from character profiles (`canonicalFaceRef`/`canonicalBodyRef`), calls the renderer directly, persists the result, and patches `renderResultId`. The `render_panels` step skips panels that already have a `renderResultId`, so regenerating via the API doesn't require re-running the entire step. The renderer handles `referenceImageKeys` for image-to-image conditioning (aisdk uses OpenAI `/v1/images/edits`). No PipelineActor action needed — the direct API route is the right architecture for single-panel renders.
17. ✅ **Fix `Effect.orDie` overuse** (COMPLETED 2026-06-29) — Added `logAndDie(label)` curried combinator to `packages/actors/src/lib/services.ts` that logs the cause via `Effect.logError` before dying. Replaced all 22 `Effect.orDie` calls in `pipeline/live.ts` with `logAndDie("descriptive.label")` — state reads/writes now log schema-mismatch defects with context instead of vanishing silently. Other actors (bible, chapter, project, file-registry, knowledge-base) retain `orDie` as their schema surface is smaller and less critical.
