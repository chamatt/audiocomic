...
export const ChapterLive = Chapter.toLayer(
  ({ rawRivetkitContext, state }) =>
    Effect.gen(function* () {
      ...
      const getState = (): Effect.Effect<ChapterState> => State.get(state).pipe(Effect.orDie);

      const update = (fn: (current: ChapterState) => ChapterState): Effect.Effect<ChapterState> =>
        State.updateAndGet(state, fn).pipe(
          Effect.orDie,
          Effect.tap((next) =>
            Effect.sync(() => rawRivetkitContext.broadcast("chapterUpdated", next)),
          ),
        );

      // Update stage + progress in actor state, DB, and broadcast.
      const setStage = (stage: string, progress?: StageProgress): Effect.Effect<ChapterState> =>
      ...
      const runIngest = (current: ChapterState): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          ...
          yield* Effect.tryPromise({
            try: () =>
              ingestChapterTranscription(bridge.repo, embedder, current.projectId, current.id),
            catch: (e) => (e instanceof Error ? e : new Error(String(e))),
          }).pipe(
            Effect.tap((r) =>
              Effect.sync(() => {
                embeddingsCount = r.embeddingCount;
              }),
            ),
            Effect.tap(() =>
              Effect.sync(() =>
                log.info("embedding done", { chapterId: current.id, embeddings: embeddingsCount }),
              ),
            ),
            Effect.catchCause((cause) =>
              Effect.gen(function* () {
                log.warn("embedding skipped (non-fatal)", {
                  chapterId: current.id,
                  error: cause.toString(),
                });
                yield* setStage("ingesting", {
                  current: 1,
                  total: 3,
                  detail: "Embeddings skipped — continuing with wiki",
                });
              }),
            ),
          );
          ...
          yield* setStage("ingesting", {
            current: 1,
            total: 3,
            detail: "Extracting wiki entities",
          });
          ...
          yield* Effect.tryPromise({
            try: () =>
              wiki.ingestChapter(current.id, current.projectId, chapterText, current.index),
            catch: (e) => (e instanceof Error ? e : new Error(String(e))),
          });
          ...
          yield* Effect.tryPromise({
            try: () =>
              bridge.repo.chapterIngestLog.insert({
                chapterId: current.id,
                projectId: current.projectId,
                embeddingsCount,
          ...
          Effect.catchCause((cause) =>
            Effect.gen(function* () {
              ...
              yield* Effect.tryPromise({
                try: () =>
                  bridge.repo.chapters.patch(current.id, { status: "failed" } as Partial<unknown>),
                catch: () => Promise.resolve(),
              }).pipe(Effect.ignore);
              rawRivetkitContext.broadcast("chapterIngestFailed", {
                chapterId: current.id,
                error: cause.toString(),
              });
            }),
      ...
      const runPlan = (current: ChapterState): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          ...
          yield* setStage("planning", {
            current: 1,
            total: 4,
            detail: "Planning story (3-pass: scenes → beats → panels)",
          });
          ...
          if (beats.length === 0) {
            ...
            log.warn("no beat-level sections found, using all sections", {
              chapterId: current.id,
          ...
          if (effectiveBeats.length === 0) {
            yield* setStage("failed", {
              current: 0,
              total: 0,
              detail: "No sections extracted from story plan",
            });
            return;
          }
          ...
          yield* setStage("ready_for_review", {
            current: pages.length,
            total: pages.length,
            detail: `${pages.length} pages, ${panels.length} panels`,
          });
          ...
          rawRivetkitContext.broadcast("chapterPlanned", {
            chapterId: current.id,
            pages: pages.length,
            panels: panels.length,
          });
          ...
          Effect.catchCause((cause) =>
            Effect.gen(function* () {
              ...
              yield* setStage("failed", { current: 0, total: 0, detail: cause.toString() });
              rawRivetkitContext.broadcast("chapterPlanFailed", {
                chapterId: current.id,
                error: cause.toString(),
              });
            }),
      ...
      const runRender = (current: ChapterState): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          ...
          const panelsToRender = chapterPanels.filter(
            (p) => p.renderPrompt && !p.renderResultId,
          ) as PanelSpec[];

          yield* setStage("rendering", {
            current: 0,
            total: panelsToRender.length,
            detail: `Rendering ${panelsToRender.length} panels`,
          });
          log.info("rendering panels", {
            chapterId: current.id,
            toRender: panelsToRender.length,
          ...
          for (let i = 0; i < panelsToRender.length; i++) {
            ...
            yield* setStage("rendering", {
              current: i,
              total: panelsToRender.length,
              detail: `Panel ${i + 1}/${panelsToRender.length}`,
            });
          ...
          yield* setStage("rendering", {
            current: panelsToRender.length,
            total: panelsToRender.length,
            detail: "All panels rendered",
          });
          ...
          Effect.catchCause((cause) =>
            Effect.gen(function* () {
              ...
              yield* setStage("failed", { current: 0, total: 0, detail: cause.toString() });
              rawRivetkitContext.broadcast("chapterRenderFailed", {
                chapterId: current.id,
                error: cause.toString(),
              });
            }),
        ...
        Effect.gen(function* () {
          ...
          for (const page of chapterPages) {
            ...
            const composed = yield* Effect.tryPromise({
              try: () =>
                bridge.composePage(panelImages, page, pagePanels, { width: 1200, height: 1600 }),
              catch: (e) => (e instanceof Error ? e : new Error(String(e))),
            });
          ...
          for (const page of chapterPages) {
            ...
            yield* Effect.tryPromise(() => bridge.storage.writeAsset(key, Buffer.from(svg)));
          ...
          Effect.catchCause((cause) =>
            Effect.gen(function* () {
              ...
              yield* setStage("failed", { current: 0, total: 0, detail: cause.toString() });
              rawRivetkitContext.broadcast("chapterComposeFailed", {
                chapterId: current.id,
                error: cause.toString(),
              });
            }),
      ...
      const runTranscription = (current: ChapterState): Effect.Effect<void, Error> =>
        Effect.gen(function* () {
          ...
          if (!assetId) {
            yield* Effect.fail(new Error("StartTranscription: no sourceAssetId linked"));
          }
          ...
          if (!asset) {
            yield* Effect.fail(new Error(`StartTranscription: source asset ${assetId} not found`));
          }
          ...
          log.info("downloaded audio from storage", {
          ...
          const rawTmpPath = join(
            tmpdir(),
            `chapter-${current.id}-${uuid()}.${asset!.filename.split(".").pop() ?? "raw"}`,
          );
          yield* Effect.tryPromise({
          ...
          try {
            ...
            log.info("transcription adapter returned", {
            ...
            log.info("persisted transcript chunks", {
            ...
            log.info("chapter transcription completed", {
              chapterId: current.id,
              status: done.status,
            });
          ...
          Effect.catchCause((cause) =>
            Effect.gen(function* () {
              log.error("transcription fiber failed", {
                chapterId: current.id,
                error: cause.toString(),
              });
              ...
              rawRivetkitContext.broadcast("chapterTranscriptionFailed", {
                chapterId: failed.id,
                error: cause.toString(),
              });
      ...
      return Chapter.of({
        ...
        SetStage: ({ payload }) => setStage(payload.stage, payload.progress),

        StartIngest: () =>
          Effect.gen(function* () {
            const current = yield* getState();
            const running = yield* setStage("ingesting", {
              current: 0,
              total: 3,
              detail: "Starting",
            });
        ...
        StartPlan: () =>
          Effect.gen(function* () {
            const current = yield* getState();
            const running = yield* setStage("planning", {
              current: 0,
              total: 4,
              detail: "Starting",
            });
        ...
        StartRender: () =>
          Effect.gen(function* () {
            const current = yield* getState();
            const running = yield* setStage("rendering", {
              current: 0,
              total: 0,
              detail: "Starting",
            });
        ...
        StartCompose: () =>
          Effect.gen(function* () {
            const current = yield* getState();
            const running = yield* setStage("composing", {
              current: 0,
              total: 2,
              detail: "Starting",
            });
