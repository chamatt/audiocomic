		...
		Effect.gen(function* () {
			...
			if (transcribed.length === 0) {
				return yield* Effect.fail(
					new Error("plan_chapters: no transcribed chapters — upload and transcribe chapters first"),
				);
			}
			...
			for (let chIdx = 0; chIdx < transcribed.length; chIdx++) {
				...
				if (chapterText.length === 0) {
					yield* Effect.logWarning(`plan_chapters: chapter ${chapter.id} has no transcript text, skipping`);
					continue;
				}
				...
				if (beats.length === 0) {
					yield* Effect.logWarning(`plan_chapters: no beats for chapter ${chapter.id}, skipping pages`);
					continue;
				}
